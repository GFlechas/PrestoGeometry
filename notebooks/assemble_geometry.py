...
class AssemblyEditor:
    ...
    def _build_ui(self):
        ...
        # Bottom button bar
        bh, by = 0.042, 0.008
        ax_solve  = self.fig.add_axes([0.03,  by, 0.12, bh])
        ax_reset  = self.fig.add_axes([0.165, by, 0.12, bh])
        ax_save   = self.fig.add_axes([0.30,  by, 0.12, bh])
        ax_convex = self.fig.add_axes([0.435, by, 0.12, bh])

        self._btn_solve  = Button(ax_solve,  'Solve',
                                  color='#1a4a1a', hovercolor='#2a7a2a')
        self._btn_reset  = Button(ax_reset,  'Reset 90 deg',
        ...
        for b in (self._btn_solve, self._btn_reset,
                  self._btn_save, self._btn_convex):
        ...
        # Status label
        self._status_ax = self.fig.add_axes([0.57, by, 0.42, bh])
...
def main():
    ...
    if args.no_interactive:
        ...
        print("  Click green corners to mark them FREE (yellow).")
        print("  Click [Solve] to find the angles that close the polygon.")
        print("  Click [Save PNG] when satisfied.")
        editor.run()

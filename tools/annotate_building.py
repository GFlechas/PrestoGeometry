...
def main():
    ...
    parser.add_argument(
        ...
        help='Building name (used for annotation file names)')
    parser.add_argument(
        '--photos-dir', type=str, default='',
        dest='photos_dir',
        help='Path to folder of photos.  '
             'Defaults to photos/<building> inside the repo.')
    parser.add_argument(
    ...
    if args.photos_dir:
        ...
        photo_dir = REPO_ROOT / 'photos' / args.building

    if not photo_dir.exists():
        print(f'ERROR: Photo directory not found: {photo_dir}')
        sys.exit(1)

    n_images = len([p for ext in ('*.jpg','*.jpeg','*.JPG','*.JPEG','*.png','*.PNG')
                    for p in photo_dir.glob(ext)])

    print(f"\n{'='*60}")
    print(f"  ANNOTATION TOOL  —  {args.building}")
    print(f"{'='*60}")
    print(f"  Photos folder : {photo_dir}")
    print(f"  Images found  : {n_images}")
    print(f"  Faces active  : {args.faces}  (keys 1–{min(args.faces, 8)} to switch)")
    print(f"  Annotations   : {ANNOTATIONS_DIR / args.building / (args.building + '.json')}")
    print(f"\n  GETTING STARTED")
    print(f"  ──────────────────────────────────────────────────")
    print(f"  1. The first image opens automatically.")
    print(f"  2. Press 1–8 to select the active face (Face 0–7).")
    print(f"     Face 0 is selected by default (shown in the sidebar).")
    print(f"  3. Left-click to start drawing segments along a face edge.")
    print(f"     Each click chains to the next — trace all four sides")
    print(f"     of the face (top, bottom, left edge, right edge).")
    print(f"  4. At building corners visible in the photo, SNAP your")
    print(f"     segment endpoint onto an endpoint of the adjacent face.")
    print(f"     A yellow ring appears when you are close enough to snap.")
    print(f"     This tells the assembler which faces are neighbours.")
    print(f"  5. Press Space or → to save and move to the next image.")
    print(f"     Press ← or B to go back.")
    print(f"  6. Press H at any time to show the full control reference.")
    print(f"  7. Press Q when done — annotations save automatically.")
    print(f"\n  TIP: You do not need to annotate every photo.  Annotate")
    print(f"  the images that show each face most clearly.")
    print(f"{'='*60}\n")
